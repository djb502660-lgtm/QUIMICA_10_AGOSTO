...
import { Mic, MicOff, AlertCircle, Sparkles, User2, Heart, Zap, Send, MessageSquare, Sun, Moon, Home, MonitorUp, MonitorOff, X, Volume2, Key } from 'lucide-react';
...
import Conocimiento_Planificacion_Microcurricular_Quimica from './PensamientosPEA/Planificacion_Microcurricular_Quimica.md?raw';
...
const ASSISTANTS: Assistant[] = [
  {
    ...
    systemInstruction: `
      ...
      ${Conocimiento_Planificacion_Microcurricular_Quimica}
...
const App: React.FC = () => {
  ...
  // Auto-reconectar en error 1011 o 1006, o abrir modal si es error de API Key
  useEffect(() => {
    if (connectionState === ConnectionState.DISCONNECTED && errorMessage) {
      if (errorMessage.toLowerCase().includes("api key") || errorMessage.includes("1008") || errorMessage.toLowerCase().includes("not valid")) {
        setShowApiKeyModal(true);
      } else if ((errorMessage.includes("1011") || errorMessage.includes("1006")) && reconnectAttempts < 3) {
    ...
    <div className={`min-h-screen ${isDarkTheme
      ...
      <div className="relative z-10 w-full max-w-5xl flex flex-col lg:flex-row items-center justify-center gap-8 lg:gap-16">
        ...
        <div className="w-full max-w-lg flex flex-col items-center gap-4">
          ...
          {/* Encabezado */}
          <div className="text-center relative w-full flex items-center justify-center gap-3">
            {/* Botones Izquierda: Home y Tema */}
            <div className="absolute left-0 top-1/2 -translate-y-1/2 flex items-center gap-1">
              <button
                onClick={() => setView('dst_menu')}
                className={`p-2 rounded-full transition-colors ${isDarkTheme ? 'hover:bg-white/10 text-slate-400' : 'hover:bg-slate-200 text-slate-600'}`}
            ...
            {/* Badge central */}
            <div className={`inline-flex items-center gap-2 px-4 py-2 rounded-full border backdrop-blur-sm transition-colors duration-500 ${isDarkTheme ? 'border-white/10' : 'border-slate-300'} ${currentTheme.bgBadge}`}>
              <Sparkles className={`w-4 h-4 ${currentTheme.accent}`} />
              <span className={`text-sm font-medium tracking-wide uppercase ${isDarkTheme ? currentTheme.text : 'text-slate-700'}`}>Gemini 3.1 Flash Live (PREVIEW)</span>
            </div>

            {/* Botones Derecha: Mascotas, API Key, Chat */}
            <div className="absolute right-0 top-1/2 -translate-y-1/2 flex items-center gap-1">
              <img src="/assets/mascotas10_agosto_istae.png" alt="Mascotas" className="w-10 h-10 object-contain" />
              <button
                onClick={() => setShowApiKeyModal(true)}
                className={`p-2 rounded-full transition-colors relative ${isDarkTheme ? 'hover:bg-white/10 text-slate-400' : 'hover:bg-slate-200 text-slate-600'}`}
              ...
              <button
                ...
                className={`p-2 rounded-full hover:bg-white/10 transition-colors ${isDarkTheme ? 'text-slate-400' : 'text-slate-600'}`}
          ...
          {/* Nombre y Descripción del Asistente */}
          <div className="text-center space-y-1 mt-1">
            ...
            <h1 className="text-4xl lg:text-5xl font-bold bg-gradient-to-r from-white to-slate-400 bg-clip-text text-transparent transition-all duration-500">
              {selectedAssistant.name}
            </h1>
            <p className="text-slate-400 text-sm transition-opacity duration-300 max-w-xs mx-auto">
          ...
          `}>
            ...
            <div className={`absolute inset-0 transition-opacity duration-700 flex items-center justify-center ${isConnected ? 'opacity-40' : 'opacity-95'}`}>
              <img
                src={selectedAssistant.id === 'Sofia' ? '/sofia.jpg' : selectedAssistant.id === 'Agostin' ? '/assets/mascota10_agosto.png.png' : `https://picsum.photos/seed/${selectedAssistant.id}/400/400`}
                alt="Avatar"
                className={`w-full h-full transition-all duration-1000 ${
                  selectedAssistant.id === 'Agostin'
                    ? 'object-contain p-3 scale-95 drop-shadow-2xl'
                    : 'object-cover'
                }`}
              />
            </div>
          ...
          <div className="flex flex-col items-center gap-6 w-full mt-2">
            {errorMessage && !isConnected && (
              <div className="flex items-center gap-2 text-red-400 bg-red-900/20 px-4 py-3 rounded-xl border border-red-900/50 text-sm w-full animate-in fade-in slide-in-from-bottom-2 justify-between">
                ...
                {(errorMessage.toLowerCase().includes("api key") || errorMessage.includes("1008") || errorMessage.toLowerCase().includes("not valid")) && (
                  <button
                    onClick={() => setShowApiKeyModal(true)}
                    className="px-3 py-1 bg-red-500/20 hover:bg-red-500/30 text-red-200 border border-red-500/30 rounded-lg text-xs font-medium transition-colors shrink-0 ml-2"
                  >
                    Ingresar Key
                  </button>
                )}
